...
import com.onework.boot.module.system.core.controller.vo.DictionaryPageReqVO;
import com.onework.boot.module.system.core.controller.vo.DictionaryReqVO;
...
public class DictionaryController {
    ...
    public CommonResult<Dictionary> addOrUpdate(@RequestBody Dictionary params) {
    ...
    @GetMapping("get/{id}")
    public CommonResult<Dictionary> get(@PathVariable String id) {
        return CommonResult.success(service.getById(id));
    }

    @Operation(description = "查询列表", summary = "查询列表")
    @GetMapping("getAll")
    public CommonResult<List<Dictionary>> getAll(@Validated DictionaryReqVO reqVO) {
        LambdaQueryWrapperX<Dictionary> queryWrapper = new LambdaQueryWrapperX<>();
        List<Dictionary> resultData = service.list(queryWrapper);
        return CommonResult.success(resultData);
    }

    @Operation(description = "分页查询", summary = "分页查询")
    @GetMapping("getPageAll")
    public CommonResult<PageResult<Dictionary>> getPageList(@Validated DictionaryPageReqVO pageReqVO) {

        Page<Dictionary> page = MyBatisUtils.buildPage(pageReqVO);
        LambdaQueryWrapperX<Dictionary> queryWrapper = new LambdaQueryWrapperX<>();
        long total = service.count(queryWrapper);
        List<Dictionary> data = service.list(page, queryWrapper);
        PageResult<Dictionary> pageResult = new PageResult<>(data, total);
