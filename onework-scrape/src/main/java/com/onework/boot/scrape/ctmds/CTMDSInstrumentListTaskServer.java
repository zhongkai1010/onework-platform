...
@Component
public class CTMDSInstrumentListTaskServer implements ITaskServer {
    ...
    public CTMDSInstrumentListTaskServer(ScrapeConfiguration scrapeConfiguration, CTMDSRecordStore ctmdsRecordStore) {
    ...
    @Override
    public void run() {
        String json = HttpRequest.post("https://beian.cfdi.org.cn/CTMDS/pub/PUB010100.do?method=handle05&pageSize=10000").execute().body();
        ...
        ScrapeHelper.listWorkerExecute(institutions, 50, (start, end, items) -> {
            for (int i = 0; i < items.size(); i++) {
                ...
                try {
                    ...
                    String html = HttpRequest.get(String.format("https://beian.cfdi.org.cn/CTMDS/pub/PUB010100.do?compId=%s&method=handle04", institution.getCompanyId())).execute().body();
                    String filePathName = String.format("%s\\械-%s.html", scrapeConfiguration.getCtmdsSavePath(), institution.getCompName());
